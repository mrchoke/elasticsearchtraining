...
OLDUID=$(id elasticsearch -u)

usermod -u $DUID elasticsearch
find /usr -user $OLDUID -exec chown -h $DUID {} \;
find /var -user $OLDUID -exec chown -h $DUID {} \;


run_as_other_user_if_needed() {
  if [[ "$(id -u)" == "0" ]]; then
    # If running as root, drop to specified UID and run command
    exec chroot --userspec=$DUID / "${@}"
  else
